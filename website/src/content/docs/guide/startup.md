...
While it is running, a small controller icon is available in the Windows notification area. Right-click it to open the logs, the readable local input-usage report, or configuration folder, or to exit CouchPilot cleanly. Windows may initially place a new icon under the overflow arrow.
