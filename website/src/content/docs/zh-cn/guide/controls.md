...
Codex、QQ/微信、Claude 和 Cherry Studio 会增加一个临时语音编辑状态。按 <kbd>Y</kbd> 说话，轻按或长按 <kbd>B</kbd> 删除字符，再按 <kbd>A</kbd> 发送。只要移动鼠标，就会退出这个状态，并立刻恢复 A/B 的普通功能。

每个 App 页都会写明具体行为和安全边界。浏览器页面、文档和终端不会启用语音发送。
