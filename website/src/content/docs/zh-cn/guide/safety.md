...
正常使用时，<kbd>A</kbd> 始终是鼠标左键。在 Codex 中按 <kbd>Y</kbd> 启动语音输入后，CouchPilot 才会临时把 <kbd>A</kbd> 映射为 Enter，让你主动确认发送。移动鼠标、切换 App、完成发送或超过配置的等待时间，都会恢复普通鼠标操作。

浏览器以及 Codex 以外的所有 App 都不会进入这个状态。
